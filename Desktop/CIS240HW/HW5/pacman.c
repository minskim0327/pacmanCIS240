...
void draw_score ()
{
  ...
  **/
  ...
  // the inside
  // the number of starting points vary on the numbers of digits
  // if digit = 1, score_right starts at (60, 56)
  // if digit = 2, score_middle starts at (55, 56), score_right starts at(64, 56)
  // there are no possible cases for digit being 3, because maximum score is number of dots = 92
  // unsigned int *adr_score_left = zero + 8 * score_left;
  unsigned int *adr_score_middle = zero + 8 * score_middle;
  unsigned int *adr_score_right = zero + 8 * score_right;

  // when the score has a single digit, 
  if (score_middle == 0){
    if (score_right == 0) {
      lc4_draw_sprite(60, 56, WHITE, zero);
    }
    if (prev_score_right != score_right) {
      lc4_draw_rect(60, 56, 8, 8, BLACK);
      lc4_draw_sprite(60, 56, WHITE, adr_score_right);
    }
  }
  // when the score has a double digit.
  else if (score_left == 0) {
    // delete the previous drawing of one-digit score
    if(prev_score_middle == 0) {
      lc4_draw_rect(60, 56, 8, 8, BLACK);
    }

    // when the second digit value changes 
    if (prev_score_middle != score_middle) {
      lc4_draw_rect(55, 56, 8, 8, BLACK);
      lc4_draw_rect(64, 56, 8, 8, BLACK);
      lc4_draw_sprite(55, 56, WHITE, adr_score_middle);
      lc4_draw_sprite(64, 56, WHITE, adr_score_right); 
    } else if (prev_score_right != score_right) {
        lc4_draw_rect(64, 56, 8, 8, BLACK);
	lc4_draw_sprite(64, 56, WHITE, adr_score_right);
    }
  }
  ...
  prev_score_middle = score_middle;
...
void redraw () {
  ...
  **/
  ...
  /* Redraw the Pacman */
  // remove the previous pacman (original position: prevx, prevy, fill with 8 by 8 black box)
  // draw the current pacman (new position: x, y, fill with Yellow color and the shape determined by direction)
  ...
  /* Redraw the ghosts */
  // remove the original ghost
  // draw the current ghost
  for (i = 0; i < numghosts; i++){
    if (ghostsprevx[i] != ghostsx[i] || ghostsprevy[i] != ghostsy[i]){ 
      lc4_draw_rect(ghostsprevx[i], ghostsprevy[i], 8, 8, BLACK);
      lc4_draw_sprite(ghostsx[i], ghostsy[i], colors[i%2], ghost);
    }
  }

  /* Redraw the dots */
  // iterate through 92 dots
  // if it is not eaten, draw them
  for (i = 0; i < numdots; i++){
...
void choose_direction(int ghost) {
  ...
  **/
  ...
  /**
   * store the previous direction of the ghost(prev_direction)
   * determine whether prev_direction is vertical(UP or DOWN) or horizontal(LEFT or RIGHT)
   * compare its relative location to the pacman
   * determine the direction
  */
  int prev_direction = ghostsdirection[ghost];
  if (prev_direction == UP || prev_direction == DOWN){
     // compare value of x coordinate of ghost and pacman
     if (x < ghostsx[ghost]){
	ghostsdirection[ghost] = LEFT;
     } else if (x > ghostsx[ghost]) {
	ghostsdirection[ghost] = RIGHT;
     }
  }
  if (prev_direction == LEFT || prev_direction == RIGHT){
     // compare value of y coordinate of ghost and pacman
     if (y < ghostsy[ghost]) {
	ghostsdirection[ghost] = UP;
     } else if (y > ghostsy[ghost]) {
...
void choose_direction_random(int ghost) {
  ...
  **/
  ...
  // YOUR CODE HERE
  /**
   * UP = 1, DOWN = 2, RIGHT = 3, LEFT = 4 as defined
   * rand16() returns a pseudo-random integer from 0 to 127
   * re-define UP = 0, DOWN = 1, RIGHT = 2, LEFT = 3 for mod 4 usage
   * choose a random direction until it is different from the previous direction
   */
  int rand_dir = rand16() % 4;
  int curr_dir = ghostsdirection[ghost];
  while (curr_dir - 1 == rand_dir){
	  rand_dir = rand16() % 4;
  }
  ghostsdirection[ghost] = rand_dir + 1;
}
...
void move_ghost(int ghostnumb, int is_random) {
  ...
  **/
  ...
  // YOUR CODE HERE
  // When ghosts are moving up
  top = ghostsy[ghostnumb];
  bottom = top + 7;
  left = ghostsx[ghostnumb];
  right = left + 7;

  if (ghostsdirection[ghostnumb] == UP){
     for (j = 1; j < 5; j++) {
	for (i = 0; i < 8; i++) {
	  if (top - j >= 0 && validspace[top - j][left + i] == 1){
	    ghostsy[ghostnumb] = top - j + 1;
	    if (is_random) { choose_direction_random(ghostnumb); }
	    else { choose_direction(ghostnumb); }
	    return;
	  }
	}
     }
  ...
  if (ghostsdirection[ghostnumb] == DOWN){
     for (j = 1; j < 5; j++) {
	for (i = 0; i < 8; i++) {
	  if (bottom + j < 124 && validspace[bottom + j][left + i] == 1){
	    ghostsy[ghostnumb] = bottom + j - 8;
	    if (is_random) { choose_direction_random(ghostnumb); }
	    else { choose_direction(ghostnumb); }
	    return;
	  }
  ...
  if (ghostsdirection[ghostnumb] == LEFT){
     for (j = 1; j < 5; j++) {
	for (i = 0; i < 8; i++) {
	  if (left - j >= 0 && validspace[top + i][left - j] == 1){
	    ghostsx[ghostnumb] = left - j + 1;
	    if (is_random) { choose_direction_random(ghostnumb); }
	    else { choose_direction(ghostnumb); }
	    return;
	  }
  ...
  if (ghostsdirection[ghostnumb] == RIGHT){
     for (j = 1; j < 5; j++) {
	for (i = 0; i < 8; i++) {
	  if (right + j < 128 && validspace[top + i][right + j] == 1){
	    ghostsx[ghostnumb] = right + j - 8;
	    if (is_random) { choose_direction_random(ghostnumb); }
	    else { choose_direction(ghostnumb); }
	    return;
	  }
  ...
  if (ghostsx[ghostnumb] + 7 < 2) {
     ghostsx[ghostnumb] = 126;
     ghostsdirection[ghostnumb] = LEFT;
     return;
  } else if (ghostsx[ghostnumb] > 126) {
